...
After composing the modelcar / base / model relationships, Mobster scans the
**base image** with Syft and merges that package inventory under the base image
node (`CONTAINS` in SPDX, `dependencies` in CycloneDX). Scan the base, not the
assembled ModelCar with weight layers.
...
The example composition SBOMs (before Syft inventory) are available in:
