...
async def load_sbom_from_json(file_path: Path) -> dict[str, Any]:
    ...
    Args:
        file_path (Path): Path to the JSON SBOM file (SPDX 2.X or CycloneDX 1.5+)
    Returns:
        dict[str, Any]: The SBOM dictionary from the file.
    """
    with open(file_path, encoding="utf-8") as in_stream:
        try:
            return json.load(in_stream)  # type: ignore[no-any-return]
        except JSONDecodeError:
            # Return the cursor to the beginning
            in_stream.seek(0)
            LOGGER.critical("Expected a JSON SBOM. Found different file contents!")
            LOGGER.critical(in_stream.read())
            raise
