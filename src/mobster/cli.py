...
from mobster.cmd import augment, enrich
...
def generate_oci_image_parser(subparsers: Any) -> None:
    ...
    oci_image_parser.add_argument(
        "--metadata-path",
        type=Path,
        help="Path to a metadata yaml file.",
    )
    oci_image_parser.add_argument(
        "--arch",
        type=normalize_arch,
...
def generate_modelcar_parser(subparsers: Any) -> None:
    ...
    modelcar_parser.add_argument(
        ...
        type=Path,
        help=(
            "Path to an SPDX SBOM generated by Syft (typically a base-image "
            "scan). Packages are merged into the modelcar composition SBOM "
            "with CONTAINS relationships. Only supported with --sbom-type spdx."
        ),
        action="append",
    )
...
def enrich_command_parser(subparsers: Any) -> None:
    ...
    enrich_aibom_parser(enrich_subparsers)


def enrich_aibom_parser(subparsers: Any) -> None:
    """
    A parser for augmenting SBOMs for OCI images.
    """
    ...
    aibom_parser = subparsers.add_parser(
        "ai-bom",
        help="enrich an SBOM with AI information from a CycloneDX SBOM",
    )
    ...
    aibom_parser.set_defaults(func=enrich.EnrichCommand)
