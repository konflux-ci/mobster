...
async def normalize_sbom(sbom: dict[str, Any], append_mobster: bool = True) -> None:
    """
    Adds necessary fields to an SPDX SBOM to be loaded by the
    SPDX library without validation issues.
    Args:
        sbom (dict[str, Any]): The SBOM to be normalized.
        append_mobster (bool): If Mobster should append its name as one of
...
async def normalize_and_load_sbom(
    ...
    Args:
        sbom (dict[str, Any]): The SBOM dict to normalize and load.
        append_mobster (bool): If Mobster should append its name as one of
                               the creators of the SBOM.
    Returns:
        spdx_tools.spdx.model.document.Document: Loaded SPDX SBOM object.
    """
