...
class GenerateOciImageCommand(GenerateCommandWithOutputTypeSelector):
    ...
    async def _execute_contextual_workflow(
        ...
        Args:
            component_sbom_doc (spdx_tools.spdx.model.document.Document):
                The component SBOM created for this image.
            parent_image_ref (str): Reference to the parent image.
            arch (str): CPU architecture of this image.
        ...
        if self.cli_args.from_hermeto:
            hermeto_dict = await load_sbom_from_json(self.cli_args.from_hermeto)
            hermeto_doc = await normalize_and_load_sbom(hermeto_dict)

        parent_image_sbom = await download_parent_image_sbom(parent_image_ref, arch)
        if not parent_image_sbom or not use_contextual_sbom_creation(parent_image_sbom):
            return None
        ...
        return await create_contextual_sbom(
            parent_sbom_doc, component_only_sbom_doc, hermeto_doc
        )
    ...
    async def _assess_and_dispatch_contextual_workflow(
        ...
        Args:
            component_sbom_doc (spdx_tools.spdx.model.document.Document):
                The component SBOM created for this image.
            base_images_refs (list[str]): List of references from the parsed
                Dockerfile.
            image_arch (str): CPU architecture of this image.
        ...
        if (
            ...
            try:
                return await self._execute_contextual_workflow(
                    component_sbom_doc, parent_image_ref, image_arch
                )
    ...
    async def execute(self) -> Any:
        ...
        contextual_sbom = await self._assess_and_dispatch_contextual_workflow(
            sbom, base_images_refs, image_arch
        )
