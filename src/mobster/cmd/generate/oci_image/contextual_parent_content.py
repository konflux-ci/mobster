...
async def convert_to_descendant_of_relationship(
    ...
    if not original_relationship_type == RelationshipType.BUILD_TOOL_OF:
        ...
        return sbom_doc
    related_spdxid = original_relationship[0].related_spdx_element_id
    if not isinstance(related_spdxid, str):
        # This is technically a nonsense but
        # related_spdx_element_id is typed as str | None | NOASSERTION
        LOGGER.warning(
            "[Parent image content] The BUILD_TOOL_OF relationship "
            "does not have a valid related_spdx_element_id!"
        )
        return sbom_doc
    original_relationship[0].relationship_type = RelationshipType.DESCENDANT_OF
    original_relationship[0].spdx_element_id = related_spdxid
...
async def adjust_parent_image_spdx_element_ids(
    ...
    assert isinstance(parent_name_from_component_sbom, str), (
        "Cannot find parent SPDXID in component SBOM!"
    )
...
async def download_parent_image_sbom(
    ...
    _, oras_stdout, oras_stderr = await run_async_subprocess(
        ["oras", "manifest", "fetch", pullspec]
    )
    if not oras_stdout:
        LOGGER.warning(
            "Could not locate manifest of the '%s'. Raw stderr output: %s",
            pullspec,
            oras_stderr.decode(),
        )
        return None
    ...
    try:
        inspected_image = json.loads(oras_stdout)
    except JSONDecodeError:
        LOGGER.warning(
            "Invalid image manifest found, cannot parse JSON for pullspec '%s'.",
            pullspec,
        )
        return None

    cosign_command = ["cosign", "download", "sbom", pullspec]
    ...
    _, cosign_stdout, cosign_stderr = await run_async_subprocess(cosign_command)

    if not cosign_stdout:
        LOGGER.warning(
            "Could not locate SBOM. Raw stderr output: %s", cosign_stderr.decode()
        )
        return None
    try:
        result = json.loads(cosign_stdout)
        LOGGER.debug("Successfully downloaded parent SBOM for pullspec '%s'.", pullspec)
        return result  # type: ignore[no-any-return]
    except JSONDecodeError:
        LOGGER.warning(
            "Invalid SBOM found, cannot parse JSON for pullspec '%s'.", pullspec
        )
        return None
...
def use_contextual_sbom_creation(sbom_doc: dict[str, Any] | None) -> bool:
    """
...
async def calculate_component_only_content(
    ...
    """
    # TODO: Implement this in ISV-5709
    return MagicMock()
...
async def create_contextual_sbom(
    ...
    hermeto_bom: Document | None = None,
) -> Document:
    """
    Function merges the updated parent image and the component-only image.
    Args:
        updated_parent_sbom_doc (Document): The updated parent image SBOM.
        component_only_sbom_doc (Document): The component-only SBOM.
        hermeto_bom (Document | None): Hermeto SBOM if available.

    Returns:
        Document: The finished contextual SBOM.
    """
    # TODO: Implement this in ISV-5714
    return MagicMock()
