...
class GenerateOciArtifactCommand(GenerateCommandWithOutputTypeSelector):
    ...
    async def execute(self) -> Any:
        ...
        sbom = await self.to_sbom(oci_image, artifacts)

        self._content = sbom
        return self.content

    async def to_sbom(self, oci_image: Image, artifacts: list[Artifact]) -> Any:
        """
        Generate an SBOM document for oci artifact.
        ...
        if self.cli_args.sbom_type == "cyclonedx":
            return await self.to_cyclonedx(oci_image, artifacts)
        return await self.to_spdx(oci_image, artifacts)

    async def to_cyclonedx(self, oci_image: Image, artifacts: list[Artifact]) -> Any:
        """
    ...
    async def to_spdx(self, oci_image: Image, artifacts: list[Artifact]) -> Any:
        """
    ...
    async def get_artifact_relationship(
