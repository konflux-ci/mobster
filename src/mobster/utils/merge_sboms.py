...
class ComponentsMerger:
    """
    Utility class for merging SBOM components.
    """
    ...
    def merge_by_apparent_sameness(
        ...
        return [
            c.unwrap()
            for c in SBOMMerger.get_merged_components(components_a, components_b, key)
        ]
    ...
    def merge_by_prefering_hermeto(
        ...
        """
        is_duplicate_component = ComponentsMerger._get_syft_component_filter(
            hermeto_components
        )
    ...
    def _get_syft_component_filter(
        ...
        hermeto_non_registry_components = [
            ...
            if ComponentsMerger._is_hermeto_non_registry_dependency(component)
        ]
        hermeto_local_paths = {
            Path(subpath)
            for component in hermeto_sbom_components
        ...
        hermeto_indexed_components = {
            ComponentsMerger._unique_key_hermeto(component): component
            for component in hermeto_sbom_components
        }
        ...
        def component_is_duplicated(component: SBOMItem) -> bool:
            ...
            Returns:
               ...
               otherwise
            """
            key = ComponentsMerger._unique_key_syft(component)

            return (
                ComponentsMerger._is_syft_local_golang_component(component)
    ...
    def _is_syft_local_golang_component(component: SBOMItem) -> bool:
        ...
        if not purl or purl.type != "golang":
            return False
        if (subpath := purl.subpath) and not ComponentsMerger._subpath_is_version(
            subpath
        ):
            return True
    ...
    def _unique_key_syft(component: SBOMItem) -> str:
        ...
        if purl.type == "golang":
            ...
            if subpath and ComponentsMerger._subpath_is_version(subpath):
...
class SBOMMerger:
    ...
    def get_merged_components[T](
        ...
        """
        return SBOMMerger._dedupe(itertools.chain(items_a, items_b), by_key)
...
class CycloneDXMerger(SBOMMerger):
    """
    ...
    def _merge_tools_metadata(
        ...
        if isinstance(tools_a, dict):
            ...
            merged_components = ComponentsMerger.merge_by_apparent_sameness(
            ...
            sbom_a["metadata"]["tools"] = self.get_merged_components(
...
class SPDXMerger(SBOMMerger):
    ...
    def _merge_creation_info(
        ...
        creators = self.get_merged_components(
...
class SBOMMergerProcessor:
    """
    Class for processing and creating SBOM mergers.
    """
    ...
    def _create_merger(
        ...
        """
        sbom_type = SBOMMergerProcessor._detect_sbom_type(sbom_a)
        sbom_type2 = SBOMMergerProcessor._detect_sbom_type(sbom_b)
    ...
    def _merge_sboms(
        ...
        """
        merger = SBOMMergerProcessor._create_merger(
            sbom_a, sbom_b, merge_components_func
        )
    ...
    def merge_syft_and_hermeto_sboms(
        ...
        """
        syft_sbom = SBOMMergerProcessor.merge_multiple_syft_sboms(syft_sbom_paths)

        with open(hermeto_sbom_path, encoding="UTF-8") as file:
            hermeto_sbom = json.load(file)

        return SBOMMergerProcessor._merge_sboms(
            syft_sbom, hermeto_sbom, ComponentsMerger.merge_by_prefering_hermeto
        )
    ...
    def merge_multiple_syft_sboms(syft_sbom_paths: list[str]) -> dict[str, Any]:
        ...
        merge = functools.partial(
            SBOMMergerProcessor._merge_sboms,
            merge_components_func=ComponentsMerger.merge_by_apparent_sameness,
        )
...
def merge_sboms(
    ...
    if not hermeto_sbom_path:
        ...
        return SBOMMergerProcessor.merge_multiple_syft_sboms(syft_sbom_paths)
    return SBOMMergerProcessor.merge_syft_and_hermeto_sboms(
        syft_sbom_paths, hermeto_sbom_path
    )
