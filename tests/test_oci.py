...
from mobster.oci.cosign import CosignClient, RekorConfig
...
class TestCosignClient:
    ...
    def client(self) -> CosignClient:
        return CosignClient(self.verification_key, self.signing_key)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (CosignClient(), False),
            (CosignClient(Path("a")), False),
            (CosignClient(signing_key=Path("a")), True),
            (CosignClient(Path("a"), Path("b")), True),
        ],
    )
