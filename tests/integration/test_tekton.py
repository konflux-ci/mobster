...
async def create_image_with_build_sbom(
    ...
    await oci_client.attach_sbom(image, "spdx", sbom, cosign)
    return image
...
async def create_index_with_build_sbom(
    ...
    await oci_client.attach_sbom(index, "spdx", sbom, cosign)
    return index
...
async def test_process_component_sboms_happypath(
    ...
    cosign_client = CosignClient(
        verification_key=cosign_verify_key, signing_key=cosign_sign_key
    )
    data_dir = tmp_path
    result_dir = tmp_path / "results"
    result_dir.mkdir()
    ...
    image = await create_image_with_build_sbom(
        oci_client, generate_oci_image_case, repo_name, cosign_client
    )
    index = await create_index_with_build_sbom(
        oci_client, repo_with_registry, repo_name, [image], tmp_path, cosign_client
    )
    ...
    cosign_client = CosignClient(cosign_verify_key, cosign_sign_key)
    ...
    for image_digest, expected_sbom_type in (
        ...
        for release_image_repo in (f"{registry}/test", f"{registry}/anothertest"):
            sbom = await cosign_client.fetch_attested_sbom(
                Image(repository=release_image_repo, digest=image_digest),
                expected_sbom_type,
            )
            assert sbom
            assert sbom.format == expected_sbom_type
    # Clean up OCI registry from SBOMs so other tests are not interfered with
    for image_digest in (image.digest, index.digest):
        for release_image_repo in (f"{registry}/test", f"{registry}/anothertest"):
            await cosign_client.clean(
                Image(repository=release_image_repo, digest=image_digest).reference
            )


@pytest.mark.asyncio
@pytest.mark.slow
@pytest.mark.fail_slow("9m")
async def test_process_component_sboms_big_release(
    ...
    await oci_client.attach_sbom(image, "spdx", sbom.encode(), cosign_client)
        ...
        snapshot["components"].append(
            {
                ...
                "tags": [str(i)],
                "repositories": [
                    {"rh-registry-repo": f"{registry}/test", "tags": [str(i)]}
                ],
            }
        )
