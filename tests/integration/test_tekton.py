...
async def add_provenance_to_sbom(
    cosign_client: CosignSigner, sbom_ref: str, image: Image
) -> None:
    """
...
async def test_process_component_sboms_happypath(
    ...
    cosign_signer = CosignSigner(SignConfig(StaticSignConfig(sign_key=cosign_sign_key)))
    ...
    cosign_fetcher = CosignSBOMFetcher(
        VerifyConfig(static_verify_key=cosign_verify_key)
    )
