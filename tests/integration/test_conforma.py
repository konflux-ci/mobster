...
async def create_image_attestation(
    ...
    Args:
        ...
        index_sbom_ref (str): Index SBOM reference.
        child_sbom_ref (str): Child SBOM reference.
        cosign_client (CosignClient): CosignClient instance.
    """
...
async def test_oci_image_sboms_using_conforma(
    oci_client: ReferrersTagOCIClient, tmp_path: Path, cosign_keys: tuple[Path, Path]
) -> None:
    """
    ...
    cosign_client = CosignClient(
        cosign_config=CosignConfig(StaticSignConfig(sign_key=private_key_path))
    )
    ...
    await cosign_client.sign_image(f"{index_image.repository}@{index_image.digest}")
    await cosign_client.sign_image(f"{child_image.repository}@{child_image.digest}")
