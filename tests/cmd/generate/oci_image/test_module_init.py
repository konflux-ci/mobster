...
async def test_GenerateOciImageCommand__execute_contextual_workflow(
    ...
    command = GenerateOciImageCommand(
        MagicMock(from_hermeto="tests/sbom/test_merge_data/spdx/cachi2.bom.json")
    )
    parent_sbom, component_only, hermeto = await command._execute_contextual_workflow(  # type: ignore[misc]
        spdx_component_sbom, "registry.access.redhat.com/ubi8/ubi@sha256:sha", "amd64"
    )
    with open(
...
async def test_GenerateOciImageCommand__execute_contextual_workflow_no_downloaded_sbom(
    ...
    assert await command._execute_contextual_workflow(MagicMock(), "foo", "bar") is None
...
async def test_GenerateOciImageCommand__assess_and_dispatch_contextual_workflow(
    ...
    await command._assess_and_dispatch_contextual_workflow(
        MagicMock(spec=Document), ["foo:latest"], "amd64"
    )
